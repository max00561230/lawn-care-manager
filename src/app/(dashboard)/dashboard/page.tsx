...
export default function DashboardPage() {
  ...
  const stats = [
    { label: "Total Customers", value: customers.length, icon: Users, color: "bg-green-100 text-green-700" },
    { label: "Active Jobs", value: activeJobs, icon: Briefcase, color: "bg-blue-100 text-blue-700" },
    { label: "Revenue This Month", value: `$${revenueThisMonth.toLocaleString()}`, icon: DollarSign, color: "bg-yellow-100 text-yellow-700" },
    { label: "Unpaid Invoices", value: `$${unpaidInvoices.toLocaleString()}`, icon: AlertCircle, color: "bg-red-100 text-red-700" },
  ];
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <Link href="/appointments/" className="text-sm text-green-600 hover:text-green-800 flex items-center gap-1">
              View All <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          <div className="divide-y divide-gray-50">
            {todayAppts.length === 0 ? (
              <p className="px-5 py-8 text-gray-400 text-sm text-center">No appointments today</p>
            ) : (
              todayAppts.map((apt) => {
                const st = getStatusStyle(apt.status);
                return (
                  <div key={apt.id} className="px-5 py-3 flex items-center gap-4 hover:bg-green-50/50">
                    <div className="text-sm font-mono text-gray-500 w-16">{apt.start_time || "—"}</div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
              <h3 className="font-semibold text-green-900 flex items-center gap-2 text-sm">
                <CheckSquare className="w-4 h-4 text-green-600" /> Pending Tasks
              </h3>
              <Link href="/tasks/" className="text-xs text-green-600 hover:text-green-800">View all</Link>
            </div>
            <div className="divide-y divide-gray-50">
              {pendingTasks.length === 0 ? (
                <p className="px-5 py-4 text-gray-400 text-xs text-center">No pending tasks</p>
              ) : (
                pendingTasks.map((t) => (
                  <div key={t.id} className="px-5 py-2.5 flex items-center gap-2 text-sm">
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
              <h3 className="font-semibold text-green-900 flex items-center gap-2 text-sm">
                <Bell className="w-4 h-4 text-green-600" /> Reminders
              </h3>
              <Link href="/reminders/" className="text-xs text-green-600 hover:text-green-800">View all</Link>
            </div>
            <div className="divide-y divide-gray-50">
              {unreadReminders.length === 0 ? (
                <p className="px-5 py-4 text-gray-400 text-xs text-center">No new reminders</p>
              ) : (
                unreadReminders.map((r) => (
                  <div key={r.id} className="px-5 py-2.5 flex items-center gap-2 text-sm">
                    <span className="w-2 h-2 rounded-full bg-yellow-400" />
                    <span className="flex-1 truncate text-gray-700">{r.title}</span>
                    <span className="text-xs text-gray-400">{r.due_date}</span>
                  </div>
                ))
              )}
            </div>
      ...
      {next7.length > 0 && (
        <div className="bg-white rounded-lg shadow">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-green-900">Upcoming Appointments (Next 7 Days)</h2>
          </div>
                ...
                {next7.map((apt) => {
                  ...
                  return (
                    <tr key={apt.id} className="hover:bg-green-50/50">
                      ...
                      <td className="px-4 py-2">{apt.start_time || "—"}</td>
                      <td className="px-4 py-2">{apt.customer?.name || "—"}</td>
                      <td className="px-4 py-2">{apt.service?.name || apt.title}</td>
